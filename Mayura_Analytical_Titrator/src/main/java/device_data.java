...
public class device_data extends JDialog {
	...
	public device_data() {
		...
		validity_heading = new JLabel("Validity :");
		validity_heading.setFont(new Font("Times New Roman", Font.BOLD, 18));
		validity_heading.setBounds(15, 330, 139, 21);
		contentPane.add(validity_heading);

		validity_tf = new JTextField();
		validity_tf.setBounds(190, 330, 80, 30);
		contentPane.add(validity_tf);

		validity_left_heading = new JLabel("Days left :");
		validity_left_heading.setFont(new Font("Times New Roman", Font.BOLD, 17));
		validity_left_heading.setBounds(300, 335, 139, 21);
		...
		btn_browse.addActionListener(new ActionListener() {
		 ...
		 checkBox1.setBounds(15, 375, 400, 20);
		 checkBox1.setFont(new Font("Times New Roman", Font.BOLD, 16));

		 contentPane.add(checkBox1);
		 checkBox1.addItemListener(new ItemListener() {    
	             public void itemStateChanged(ItemEvent e) {                 
	            	 checkBox1.setText((e.getStateChange()==1?"User has the permission to select the trials!":"User doesn't have the permission to select the trials!"));    
	             }    
	          });    
		 
		 if(get_permission().matches("true")) {
			 checkBox1.setText("User has the permission to select the trials!");
			 checkBox1.setSelected(true);
		 }
		 else {
			 checkBox1.setText("User doesn't have the permission to select the trials!");
			 checkBox1.setSelected(false); 
		 }
		 
		btn_create_superAdmin = new JButton("Create Super Admin");
		btn_create_superAdmin.setFont(new Font("Times New Roman", Font.BOLD, 18));
		btn_create_superAdmin.setAlignmentX(Component.CENTER_ALIGNMENT);
		btn_create_superAdmin.setBounds(15, 410, 200, 32);
		contentPane.add(btn_create_superAdmin);
		btn_create_superAdmin.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				super_admin_creation.main(null);
			}
		});
		...
		btn_update.setBounds(370, 410, 100, 32);
		...
		btn_update.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String valid = validity_tf.getText();
				...
				if (inst_id.matches("") || comp_logo.matches("") || comp_name.matches("") || comp_address.matches("")
						...
						if (val <= 0 || val > 20000) {
							...
							try {
								...
								if (update == false) {
									...
									sql = "UPDATE company_data SET instrument_id = ?,company_logo = ?,company_name=?,company_address=?,validity=? WHERE Slno = ?";
					...
					try {
						audit_log_push.push_to_audit(get_date(), get_time(),"Service Engineer","Company Data Updated");
		...
		if (data_arr.length > 1) {
			...
			validity_tf.setText(data_arr[4]);
